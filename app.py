...
# Modelo da API. Pode ser ajustado via variavel de ambiente MODEL_NAME no Railway.
...
def call_claude(api_key, system_prompt, user_prompt):
    ...
    try:
        ...
        raise Exception(f"{e.code} {e.reason}: {body}")
...
"""
    ...
    try:
        ...
        return jsonify({"erro": f"A analise ficou muito longa e a resposta foi cortada. Tente um contrato menor ou selecione o tipo manualmente (desligue a deteccao automatica). Detalhe: {str(e)}"}), 500
    except Exception as e:
        err = str(e)
        if "401" in err or "authentication" in err.lower():
            return jsonify({"erro": "Chave de API invalida. Verifique a chave e tente novamente."}), 401
        if "404" in err or "not_found" in err.lower() or "not found" in err.lower():
            return jsonify({"erro": f"Modelo da API nao encontrado ({MODEL_NAME}). O nome do modelo pode ter mudado. Detalhe: {err}"}), 404
        if "429" in err:
        ...
        if "400" in err or "credit" in err.lower() or "billing" in err.lower():
            return jsonify({"erro": f"Erro na requisicao - verifique se a conta tem creditos. Detalhe: {err}"}), 400
